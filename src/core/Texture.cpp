...
Texture::StaticConstructor Texture::_;

Texture::StaticConstructor::StaticConstructor()
{
}

Texture::StaticConstructor::~StaticConstructor()
{
    Logger::LogDebug("Texture::StaticConstructor::~StaticConstructor(): clear caches");
    imageCache.clear();
    textCache.clear();
}


static Size recalculateSize(const Size &size, int w, int h)
{
    Size newSize;
    if (size.h == 0) {
        newSize.h = (size.w / w) * h;
    } else if (size.w == 0) {
        newSize.w = (size.h / h) * w;
    }
    return newSize;
}
...
Texture::Texture(std::shared_ptr<FontInterface> fontInterface,
...
{
    if (!loadText(fontInterface, graphicsInterface, fontName, text, size, color,
                  quality)) {
        Logger::LogCritical("Texture::Texture(std::shared_ptr<FontInterface>=" +
                            std::to_string((uint64_t) fontInterface.get()) +
                            " , std::shared_ptr<GraphicsInterface>=" + std::to_string((
                                        uint64_t) graphicsInterface.get()) + ", const std::string &=" +
                            fontName +
...
Texture::Texture(std::shared_ptr<ImageInterface> imageInterface,
                 std::shared_ptr<GraphicsInterface> graphicsInterface,
                 const std::string &path, const Size &size, const Rect &clip) : Texture()
{
    if (!loadImage(imageInterface, graphicsInterface, path, size, clip)) {
        Logger::LogCritical("Texture::Texture(std::shared_ptr<ImageInterface>=" +
                            std::to_string((uint64_t) imageInterface.get()) +
                            " , std::shared_ptr<GraphicsInterface>=" + std::to_string((
                                        uint64_t) graphicsInterface.get()) + "const std::string &=" +
                            path + ", const Rect &=" + size.toString() + ", const Rect &=" +
                            clip.toString() + "): Failed to load image. Texture is invalidated.");
        texture = nullptr;
    }
}
...
bool Texture::loadText(std::shared_ptr<FontInterface> fontInterface,
                       std::shared_ptr<GraphicsInterface> graphicsInterface,
                       const std::string &fontName, const std::string &text,
                       const Size &size, const Color &color, TextQuality quality)
{
    if (hasTextureCached(text, size, color)) {
        Texture::size = recalculateSize(graphicsInterface->getTextureSize(getCached(text, size, color)), w, h);
        Texture::clip = Rect();
        return true;
    }

    Rect windowSize = graphicsInterface->getWindowSize();
    ...
    if (textSurface == NULL) {
        Logger::LogError("Texture::loadText(const std::string &=" + fontName +
                         ...
                         graphicsInterface->getLastError());
        return false;
    }
    ...
    Texture::size = recalculateSize(size, w, h);
    Texture::clip = Rect();
    return true;
}

bool Texture::loadImage(std::shared_ptr<ImageInterface> imageInterface,
                        std::shared_ptr<GraphicsInterface> graphicsInterface,
                        const std::string &path, const Rect &size,
                        const Rect &clip)
{
    if (hasTextureCached(path)) {
        Texture::clip = clip;
        Size size = imageInterface->getTextureSize(getCached(path));
        Texture::size = recalculateRect({}, w, h);
        return true;
    }

    auto texture = imageInterface->loadImageFromFile(path);

    if (texture == nullptr) {
        Logger::LogError("Texture::loadImage(std::shared_ptr<ImageInterface>=" +
                         std::to_string((uint64_t) imageInterface.get()) +
                         ", std::shared_ptr<GraphicsInterface>=" +
                         std::to_string((uint64_t) graphicsInterface.get()) +
                         ", const std::string &=" + path + ", const Rect &=" + size.toString() +
                         ", const Rect &=" + clip.toString() +
                         "): Failed to load image. " + graphicsInterface->getLastError());
        return false;
    }

    texture = imageCache[path] = std::make_shared<TextureWrapper>(texture, path);
    Rect textureSize = imageInterface->getTextureSize(getCached(path));
    
    Texture::clip = clip;
    Texture::size = recalculateRect(size, textureSize.w, textureSize.h);
    return true;
}
...
void Texture::onRender(std::shared_ptr<GraphicsInterface> graphicsInterface,
                       const Rect &parentRect, bool flip)
{
    if (!texture) {
