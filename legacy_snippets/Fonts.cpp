...
TTF_Font *Fonts::registerFont(const std::string &fontFile, int size, const std::string &fontName)
{
    Core::initDeps();
    ...
    if (fontCache.find(fontName) != fontCache.end() && fontCache[fontName].find(size) != fontCache[fontName].end()) {
        Logger::LogInfo("Fonts::registerFont(const std::string &=" + fontFile + ", int=" + std::to_string(size)
                        + ", const std::string &="
                        + fontName
    ...
    if (!font) {
        Logger::LogDebug("Fonts::registerFont(const std::string &=" + fontFile + ", int=" + std::to_string(size)
                         + ", const std::string &="
                         + fontName
                         + "): Failed to open font. "
    ...
    Logger::LogDebug("Fonts::registerFont(const std::string &=" + fontFile + ", int=" + std::to_string(size)
                     ...
                     + fontName
                     + "): created font "
                     + std::to_string((unsigned long long int) fontCache[fontName][size]));
    return fontCache[fontName][size];
}
...
TTF_Font *Fonts::getFont(const std::string &fontName, int size)
{
    ...
    if (fontCache.find(fontName) == fontCache.end()) {
        Logger::LogWarning("Fonts::getFont(const std::string &=" + fontName + ", short=" + std::to_string(size)
                           + "): No font found with given name!");
        return nullptr;
    }

    if (fontCache[fontName].find(size) == fontCache[fontName].end()) {
        Logger::LogWarning("Fonts::getFont(const std::string &=" + fontName + ", short=" + std::to_string(size)
                           + "): No font found with given size!");

        if (fontFileCache.find(fontName) != fontFileCache.end()) {
            Logger::LogInfo("Fonts::getFont(const std::string &=" + fontName + ", short=" + std::to_string(size)
                            + "): Register font with cached filename '"
...
void Fonts::releaseFont(const std::string &fontName, int size)
{
    ...
    if (fontCache.find(fontName) == fontCache.end()) {
        Logger::LogWarning("Fonts::releaseFont(const std::string &=" + fontName + ", short=" + std::to_string(size)
                           + "): No font found with given name!");
        return;
    }
    ...
    Logger::LogDebug("Fonts::releaseFont(const std::string &=" + fontName + ", int=" + std::to_string(size)
                     + "): destruct font "
                     + std::to_string((unsigned long long int) fontCache[fontName][size]));
...
void Fonts::cleanup()
{
    for (auto &cache1 : fontCache) {
        for (auto &cache2 : cache1.second) {
            if (cache2.second) {
                Logger::LogDebug("Fonts::cleanup(): destruct font "
                                 + std::to_string((unsigned long long int) cache2.second));
