...
namespace bkengine
{
    ...
    class Game : public Serializable
    {
        public:
            ...
            void setInterfaces(InterfaceContainer &interfaces);
            InterfaceContainer getInterfaces() const;


            template <typename T> static T create(const InterfaceContainer & = {}, int =
                    1024, int = 768,
                                                  const std::string &title = "BKENGINE WINDOW");

            Game(const InterfaceContainer &, int width, int height,
